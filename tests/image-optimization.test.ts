...
describe("responsive image delivery", () => {
  ...
  test("adds the unrecompressed original as the largest responsive candidate", () => {
    ...
    expect(srcSet).toContain(
      "/api/images/galleries/new%20york/photo%20one_800w.webp?v=legacy-v1 800w",
    );
    expect(srcSet).toContain(
      "/api/images/galleries/new%20york/photo%20one_1600w.webp?v=legacy-v1 1600w",
    );
  ...
  test("keeps encoded paths stable for generated variants", () => {
    expect(
      ...
      "/api/images/galleries/south%20of%20spain/a%2Bb_1600w.webp?v=legacy-v1",
    );
  });
});
