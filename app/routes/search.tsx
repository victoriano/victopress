...
export const meta: MetaFunction = () => [
  { title: "Search — Victoriano Izquierdo" },
  {
    name: "description",
    content: "Search photographs by subject, place, atmosphere, or gallery.",
  },
...
export async function loader({ context }: LoaderFunctionArgs) {
  ...
  const navigation = await getNavigationFromIndex(storage);
...
async function readError(response: Response): Promise<string> {
  ...
  return `Search is temporarily unavailable (${response.status}).`;
}

export default function SearchPage() {
  const { navigation, siteName, socialLinks } = useLoaderData<typeof loader>();
  ...
  return (
    <Layout navigation={navigation} siteName={siteName} socialLinks={socialLinks}>
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
        <header className="mb-8 max-w-3xl">
          ...
          <p className="mt-3 text-sm leading-6 text-gray-500 dark:text-gray-400 sm:text-base">
            Describe a subject, place, color, or atmosphere. Search also understands related visual ideas.
          </p>
        </header>
        ...
        >
          <div>
            ...
            <input
              ...
              maxLength={300}
              defaultValue={query}
              placeholder="People at night, quiet landscapes…"
        ...
        <p className="sr-only" aria-live="polite">
          {isLoading
            ? "Searching the photo archive."
            : error
              ? `Search failed: ${error}`
              : results
                ? `${results.photos.length} search results.`
                : ""}
        </p>
        ...
        <div aria-busy={isLoading}>
          {!query && (
            <p className="py-16 text-center text-sm text-gray-400 dark:text-gray-500">
              Enter a description to explore the archive.
            </p>
          )}
          ...
          {results && results.photos.length > 0 && !isLoading && !error && (
            <>
              <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {results.photos.length} {results.photos.length === 1 ? "photograph" : "photographs"}
              ...
              <div className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {results.photos.map((photo) => (
                  <SearchResultCard key={`${photo.assetId}:${photo.path}`} photo={photo} />
                ))}
              </div>
            </>
          )}
...
function SearchResultCard({ photo }: { photo: SearchPhoto }) {
    ...
    <article className="min-w-0">
      ...
      <div className="pt-3">
        ...
        {visibleTags.length > 0 && (
          <ul className="mt-2 flex flex-wrap gap-x-2 text-[11px] text-gray-400 dark:text-gray-500" aria-label="Tags">
            {visibleTags.map((tag) => (
              <li key={tag}>#{tag}</li>
            ))}
          </ul>
